    ...
    while (available && aPtr < aEnd && bPtr < bEnd) {
        if (aPtr->y < bPtr->y) {
            ...
            *out++ = *bPtr++;
            ...
            blit(aStart, static_cast<int>(aPtr - aStart), array.data(), kBufferSize, -offset);
            if (op == Operation::Add)
                blitSrcOver(bStart, static_cast<int>(bPtr - bStart), array.data(), kBufferSize, -offset);
            else if (op == Operation::Xor)
                blitXor(bStart, static_cast<int>(bPtr - bStart), array.data(), kBufferSize, -offset);
    ...
    while (available && aPtr < aEnd && bPtr < bEnd) {
        if (aPtr->y < bPtr->y) {
            *out++ = *aPtr++;
            ...
            std::array<uchar, kBufferSize> array = {{0}};
            blit(aStart, static_cast<int>(aPtr - aStart), array.data(), kBufferSize, -offset);
            blitDestinationOut(bStart, static_cast<int>(bPtr - bStart), array.data(), kBufferSize, -offset);
            VRle::Span *tResult = temp.data();
            size_t size = bufferToRle(array.data(), kBufferSize, std::max(aLength, bLength),
                                      offset, y, tResult);
            if (available >= size) {
