...
static void gray_render_line(RAS_ARG_ TPos to_x, TPos to_y)
{
    ...
    {
        TArea prod = static_cast<TArea>(dx * fy1 - dy * fx1);
        SW_FT_UDIVPREP(dx);
        SW_FT_UDIVPREP(dy);
...
static void gray_hline(RAS_ARG_ TCoord x, TCoord y, TPos area, TCoord acount)
{
    ...
    if (coverage) {
        ...
        // update bounding box.
        if (x < ras.bound_left) ras.bound_left = static_cast<int>(x);
        if (y < ras.bound_top) ras.bound_top = static_cast<int>(y);
        if (y > ras.bound_bottom) ras.bound_bottom = static_cast<int>(y);
        if (x + acount > ras.bound_right) ras.bound_right = static_cast<int>(x + acount);
