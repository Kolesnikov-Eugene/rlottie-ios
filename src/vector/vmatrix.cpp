...
VRect VMatrix::map(const VRect &rect) const
{
    ...
    if (t <= MatrixType::Translate)
        return rect.translated(static_cast<int>(std::lround(mtx)), static_cast<int>(std::lround(mty)));

    if (t <= MatrixType::Scale) {
        int x = static_cast<int>(std::lround(m11 * rect.x() + mtx));
        int y = static_cast<int>(std::lround(m22 * rect.y() + mty));
        int w = static_cast<int>(std::lround(m11 * rect.width()));
        int h = static_cast<int>(std::lround(m22 * rect.height()));
        if (w < 0) {
            w = -w;
            x -= w;
        }
        ...
        xmax = vMax(xmax, x);
        ymax = vMax(ymax, y);
        return VRect(static_cast<int>(std::lround(xmin)), static_cast<int>(std::lround(ymin)),
                     static_cast<int>(std::lround(xmax) - std::lround(xmin)),
                     static_cast<int>(std::lround(ymax) - std::lround(ymin)));
    } else {
        // Not supported
        assert(0);
        return {};
...
VRegion VMatrix::map(const VRegion &r) const
{
    ...
    if (t == MatrixType::Translate) {
        VRegion copy(r);
        copy.translate(static_cast<int>(std::lround(mtx)), static_cast<int>(std::lround(mty)));
        return copy;
    }
