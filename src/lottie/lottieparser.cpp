...
void LottieParserImpl::parseComposition()
{
    ...
    comp->mRootLayer->mInFrame = static_cast<int>(comp->mStartFrame);
    comp->mRootLayer->mOutFrame = static_cast<int>(comp->mEndFrame);
...
std::shared_ptr<LOTData> LottieParserImpl::parseLayer(bool record)
{
    ...
    while (const char *key = NextObjectKey()) {
        if (0 == strcmp(key, "ty")) { /* Type of layer*/
            ...
            RAPIDJSON_ASSERT(PeekType() == kNumberType);
            layer->mInFrame = static_cast<int>(std::lround(GetDouble()));
        } else if (0 == strcmp(key, "op")) {
            RAPIDJSON_ASSERT(PeekType() == kNumberType);
            layer->mOutFrame = static_cast<int>(std::lround(GetDouble()));
