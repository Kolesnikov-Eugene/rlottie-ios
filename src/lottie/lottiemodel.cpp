...
void LOTGradient::populate(VGradientStops &stops, int frameNo)
{
    ...
    if (colorPoints < 0 || static_cast<size_t>(colorPoints) > size / 4) {  // for legacy bodymovin (ref: lottie-android)
        colorPoints = int(size / 4);
    }
    auto    opacityArraySize = size - colorPoints * 4;
    if ((opacityArraySize % 2 != 0) || (static_cast<size_t>(colorPoints) > opacityArraySize / 2 && opacityArraySize < 4)) {
        opacityArraySize = 0;
    }
